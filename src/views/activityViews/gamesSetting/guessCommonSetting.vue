<template>
  <sub-page>
    <div class="award-setting-contaienr">
      <main-card>
        <card-top-info title="猜电影新增设置"></card-top-info>
...
<script>
import { reqGetGuessCommon, saveGuessCommon } from '@/api/gameSetting/index';
...
export default {
  ...
  methods: {
    ...
    async initData() {
      this.isLoading = true;
      // 获取游戏数据
      const res = await reqGetGuessCommon();
      // 获取游戏配置数据
    ...
    async confirm(isReset) {
      try {
        ...
        const res = await saveGuessCommon(valueStr);
        this.isLoading = false;
        console.log(res);
        if (isReset !== 'reset') {
