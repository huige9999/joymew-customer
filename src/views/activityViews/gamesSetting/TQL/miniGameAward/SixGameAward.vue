...
import { addTQLAward, editTQLAward, addTQLAutoAward } from '@/api/tql/gameSetting';
import { getShopCouponList } from '@/api/cusshop/gameSetting';
import shopCouponItem from './components/shopCouponItem';
...
export default {
  ...
  computed: {
    ...
    Shop() {
      return this.$store.state.login.userInfo.high_privilege === 6;
    },
    kindTitle() {
      const { kind } = this;
      let title = '奖品设置';
  ...
  methods: {
    ...
    editAwardData(row) {
      ...
      console.log(row.template_id.split(','));
      this.chooseCoupon = row.template_id.split(',').map((num) => Number(num)) || [];
      if (!row.template_id) {
