<template>
  <div class="h5Theme-area">
    ...
    <div class="preview-area" v-loading="isLoading">
      ...
      <div class="themeBg" :style="{ backgroundImage: 'url(' + mpTheme + ')' }"></div>
      <img src="@/assets/image/signBoxBg.png" class="signBoxImg" />
    </div>
    <!-- 图片压缩配置区域 -->
    <imageCompress
...
export default {
  ...
  methods: {
    ...
    requestUploadImgFront(file) {
      this.isLoading = true;
      uploadImage(file, 'mobileTheme', 0.7)
        .then((res) => {
          console.log(res.key);
